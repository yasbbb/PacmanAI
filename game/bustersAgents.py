...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        l = [a for a in gameState.getLegalPacmanActions()]
        lg = gameState.getLivingGhosts()
        lgpd = [beliefs for i,beliefs
        ...
        for x in lgpd:
            ...
            dist = self.distancer.getDistance(p, highestProbLoc)
            if dist < min:
                min = dist
        ...
        min = float("inf")
        for action in l:
            dist = self.distancer.getDistance(Actions.getSuccessor(p, action),
                                                                    highestProbLoc)
            if dist < min:
                min = dist
                actionToTake = action
