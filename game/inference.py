...
def constructBayesNet(gameState: hunters.GameState):
    ...
    np_Dict[PAC]= [(x,y) for x in range(X_RANGE) for y in range(Y_RANGE)] 

    np_Dict[GHOST0] = [(x,y) for x in range(X_RANGE) for y in range(Y_RANGE)] 

    np_Dict[GHOST1]= [(x,y) for x in range(X_RANGE) for y in range(Y_RANGE)] 

    np_Dict[OBS0] = range(X_RANGE+Y_RANGE + MAX_NOISE - 1) 


    np_Dict[OBS1]= range(X_RANGE+Y_RANGE + MAX_NOISE - 1) 

    "*** END YOUR CODE HERE ***" 

    net = bn.constructEmptyBayesNet(variables, edges, np_Dict) 
    return net
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        if ghostPosition == jailPosition: #ghost in troubleee ret 1
            ...
            if noisyDistance == None:
                ...
                r = manhattanDistance(pacmanPosition, ghostPosition)
                x = busters.getObservationProbability(noisyDistance, r)
                return x
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for x in self.allPositions:
            self.beliefs[x] = self.beliefs[x]*self.getObservationProb(observation, gameState.getPacmanPosition(), x, self.getJailPosition())
        self.beliefs.normalize()  
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:    
            ...
            for newPos in newPosDist.keys():
                x[newPos] += newPosDist[newPos]*self.beliefs[oldPos]
        for pos in self.allPositions:
            self.beliefs[pos] = x[pos]
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        i = int(self.numParticles / len(self.legalPositions))
        x = self.numParticles%(len(self.legalPositions))
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        x = DiscreteDistribution()
        for particle in self.particles:
            x[particle] += self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
        if x.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [x.sample() for _ in range(len(self.particles))]
